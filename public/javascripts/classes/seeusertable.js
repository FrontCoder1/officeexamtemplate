layui.use('table', function(){
    ...
    table.render({
        elem: '#demo'
        ,cellMinWidth: 80
        ,url: '/classes/user/see/table?time='+new Date().getTime()//数据接口
        ,page: true //开启分页
        ,cols: [[ //表头
            {type: 'checkbox'}
    ...
    //用户名:username;学生姓名:name;电话:phone;开始时间:startTime终止时间:stopTime;最后一次登录时间:loginTime
                    ...
                    success: function(data){
                        if(data.state==1){
                            parent.layer.open({
                                ...
                                shade: 0.5,
                                area: ['50%', '70%'],
                                content:'/classes/user/see/updatauser?time='+new Date().getTime()//iframe的url，no代表不显示滚动条
                                ,btn: '返回'
                                ,btnAlign: 'l' //按钮居中
                                ,yes: function(){
