layui.use('table', function(){
    ...
    table.render({
        elem: '#demo'
        ,cellMinWidth: 80
        ,url: '/school/question/see/table?time='+new Date().getTime() //数据接口
        ,page: true //开启分页
        ,cols: [[ //表头
            {type: 'checkbox'}
    ...
    $('.layui-btn3').on('click', function(){
        sendajax1('GET','see/downschool/isSure','see/downschool')
    });
                    ...
                    success: function(data){
                        if(data.state==1){
                            layer.open({
                                ...
                                anim: 2,
                                scrollbar:false,
                                content:'/school/question/see/choosedetail'//iframe的url，no代表不显示滚动条
                                ,btn: '返回'
                                ,btnAlign: 'l' //按钮居中
                                ,yes: function(){
                                    layer.closeAll();
                            ...
                            //window.location.href = "/school/question/see/choosedetail";
                        }else{
                    ...
                    success: function(data){
                        if(data.state==1){
                            layer.open({
                                ...
                                anim: 2,
                                scrollbar:false,
                                content:'/school/question/see/optiondetail'//iframe的url，no代表不显示滚动条
                                ,btn: '返回'
                                ,btnAlign: 'l' //按钮居中
                                ,yes: function(){
                                    layer.closeAll();
                            ...
                            // window.location.href = "/school/question/see/optiondetail";
                        }else{
                            layer.msg(data.msg, {icon: 5});
                        }
                    }
                        ...
                        success: function(data){
                            if(data.state==1){
                                layer.open({
                                    ...
                                    anim: 2,
                                    scrollbar:false,
                                    content:'/school/question/see/updatachoose'//iframe的url，no代表不显示滚动条
                                    ,btn: '返回'
                                    ,btnAlign: 'l' //按钮居中
                                    ,yes: function(){
                                        layer.closeAll();
                                ...
                                //window.location.href = "/school/question/see/updatachoose";
                            }else{
                                layer.msg(data.msg, {icon: 5});
                            }
                        }
    ...
    function  sendajax1(method,url,downUrl) {
        $.ajax({
            url: url,
            type: method,
            dataType: "json",
